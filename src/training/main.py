...
def main(args):
    ...
    for n, p in model.named_parameters():
        if p.requires_grad:
            print(n, p.shape)
            total += p.numel()
        if "visual.vision_encoder" in n:
            v_total += p.numel()
        elif "visual.perceiver" in n:
            p_total += p.numel()
        elif "visual.reduce" in n:
            v_reduce_total += p.numel()
        elif "visual.fc" in n:
            v_fc_total += p.numel()
        elif "text.transformer" in n:
            t_total += p.numel()
        elif "text.proj" in n:
            t_fc_total += p.numel()
        # print(n, p.shape, p.dtype)

    print("visual_encoder params: ", v_total)
    print("visual reduce params: ", v_reduce_total)
    print("visual perceiver params: ", p_total)
    print("visual fc params: ", v_fc_total)
    print("text transformer params: ", t_total)
    print("text proj params: ", t_fc_total)
    print("total_trainable_params: ", total)

    # for n, p in model.named_parameters():
    #     print(n, p.device)

    # print("device of model", model.device)
    print("*" * 100)
